...
function ScanBarcode({ onLogout }: PageProps) {
    ...
    return (
        <PageLayout
            title="Stock Lookup"
            subtitle="Scan barcode to view medicine details and current stock."
            onLogout={onLogout}
        >
            <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
                <Card className="p-5">
                    ...
                    <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_auto]">
                        ...
                        <button
                            type="button"
                            onClick={() => void handleBarcodeScan(barcode)}
                            className="inline-flex h-11 items-center justify-center gap-2 self-end rounded-md bg-cyan-700 px-5 text-sm font-semibold text-white hover:bg-cyan-800"
                        >
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="rounded-md bg-slate-50 p-3">
                                ...
                                <div className="mt-1 break-all font-medium text-slate-900">{drug?.barcode || '-'}</div>
                            </div>
                            <div className="rounded-md bg-slate-50 p-3">
                                <div className="text-xs text-slate-500">Current Stock</div>
                                <div className="mt-1 text-2xl font-semibold text-slate-950">{drug?.current_stock ?? '-'}</div>
                            </div>
                            <div className="rounded-md bg-slate-50 p-3">
                                <div className="text-xs text-slate-500">Minimum Stock</div>
                                <div className="mt-1 text-2xl font-semibold text-slate-950">{drug?.min_stock ?? '-'}</div>
                            </div>
                            <div className="rounded-md bg-slate-50 p-3">
                                <div className="text-xs text-slate-500">Unit Per Scan</div>
                                <div className="mt-1 text-2xl font-semibold text-slate-950">{drug?.unit_per_scan ?? '-'}</div>
                            </div>
                        </div>
                    </div>
                </Card>
